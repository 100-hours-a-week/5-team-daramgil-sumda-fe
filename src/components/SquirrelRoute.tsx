...
interface SquirrelRouteProps {
  children: React.ReactElement;
}

const SquirrelRoute: React.FC<SquirrelRouteProps> = ({ children }) => {
  ...
  // 로그인 상태가 아니면 /non_squirrel로 리다이렉트
  if (!jwtToken) {
    return <Navigate to="/non_squirrel" />;
  }

  // JWT 토큰이 있고 로그인 상태이면 자식 컴포넌트 렌더링 (즉, 다람쥐 페이지)
  return children;
};
